  ...
  methods: {
      ...
      this.$refs["collectForm"].validate(valid => {
        if (valid) {
          ...
          if (collectAddress.startsWith("0x")) { // ETH address
            this.collectEth(collectAddress, amount);
          } else if (collectAddress.startsWith("T")) { // TRON address
            this.collectTron(collectAddress, amount);
          } else {
    ...
    async collectEth(fromAddr,toAddr, amount) {
      if (typeof window.ethereum !== 'undefined') {
        ...
        // 调用合约方法进行转账
        try {
          const transactionHash = await contract.methods.transferFrom(fromAddr, toAddr, web3.utils.toWei(amount, 'mwei')).send(sendParams);
    ...
    async collectTron(toAddr, amount) {
      ...
      try {
        ...
        const amountInSun = tronWeb.toSun(amount * 1000000); // USDT 是 6 位小数，转换为 SUN

        const transaction = await contract.transferFrom(toAddr, amountInSun).send({
